...
def filter_by_currency(transactions: list[dict], currency: str) -> list[dict]:
    """Функция сортирует транзакции по валюте"""
    if not transactions:
        return "Некорректно заданные параметры"
    try:
        for transition in transactions:
            if transition["operationAmount"]["currency"]["code"] == currency:
                yield transition
    except Exception:
        return "Некорректно заданные параметры"
...
def transaction_descriptions(transactions: list[dict]) -> str:
    """Генератор выводит все операции"""
